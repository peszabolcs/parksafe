...
export const MapboxMap: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.error = (...args) => {
      ...
      if (
        errorString.includes("ViewTagResolver") ||
        errorString.includes("view: null found with tag")
      ) {
  ...
  // Note: Tab bar spacing is now handled by the parent map.tsx container
  ...
  const [listFilter, setListFilter] = useState<
    "all" | "parking" | "repairStation" | "bicycleService"
  >("all");
  ...
  const { markers, loading, error, searchAtLocation, clearSearchResults } =
    useLocationStore();
    ...
    <ThemedView style={[styles.container, { backgroundColor }]}>
      ...
      >
        ...
        {filteredMarkers.filter((m) => m.type === "parking").length > 0 && (
          <ShapeSource
            id="parking-markers"
            shape={convertMarkersToGeoJSON(
              filteredMarkers.filter((m) => m.type === "parking")
            )}
            cluster={true}
            clusterRadius={50}
            clusterMaxZoom={14}
            onPress={(feature) => {
              if (feature.features && feature.features.length > 0) {
                ...
                const marker = markers.find(
                  (m) => m.id === markerData.properties?.id
                );
                if (marker) {
        ...
        {filteredMarkers.filter((m) => m.type === "repairStation").length >
          0 && (
          <ShapeSource
            id="repair-markers"
            shape={convertMarkersToGeoJSON(
              filteredMarkers.filter((m) => m.type === "repairStation")
            )}
            cluster={true}
            clusterRadius={50}
            clusterMaxZoom={14}
        ...
        {filteredMarkers.filter((m) => m.type === "bicycleService").length >
          0 && (
          <ShapeSource
            id="bicycle-service-markers"
            shape={convertMarkersToGeoJSON(
              filteredMarkers.filter((m) => m.type === "bicycleService")
            )}
            cluster={true}
            clusterRadius={50}
            clusterMaxZoom={14}
        ...
        {userLocationData && (
          <ShapeSource id="user-location-markers" shape={userLocationData}>
            <CircleLayer
      ...
      {/* List View Button */}
      <View style={[styles.fabGroupBottomCenter, { bottom: 30 }]}>
        <TouchableOpacity
          style={[
            styles.fabWide,
            {
